...
async function updateEmbedUrls(count: number = 50) {
  ...
  for (const movie of moviesWithoutEmbed) {
    try {
      ...
      // Fetch detailed movie information including episodes
      const { movie: movieDetail, episodes } = await phimApiClient.getMovieBySlug(movie.slug);
      ...
      const processedMovie = processMovieDetail(movieDetail, episodes);
